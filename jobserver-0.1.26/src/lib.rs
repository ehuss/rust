...
impl HelperThread {
    ...
    pub fn request_token(&self) {
        ...
        tracing::info!("ERIC: bumping requests");
        self.state.lock().requests += 1;
        tracing::info!("ERIC: notify helper");
...
impl HelperState {
    ...
    fn for_each_request(&self, mut f: impl FnMut(&HelperState)) {
        ...
        while !lock.producer_done {
            ...
            // request a token.
            if lock.requests == 0 {
                tracing::info!("ERIC: helper waiting on cvar");
                lock = self.cvar.wait(lock).unwrap_or_else(|e| e.into_inner());
                continue;
            }
            tracing::info!("ERIC: helper requests={}", lock.requests);
