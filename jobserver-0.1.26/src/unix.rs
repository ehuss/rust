...
impl Client {
    ...
    fn acquire_allow_interrupts(&self) -> io::Result<Option<Acquired>> {
        ...
        unsafe {
            ...
            loop {
                let mut buf = [0];
                tracing::info!("ERIC: reading from producer");
                match read.read(&mut buf) {
                    Ok(1) => return Ok(Some(Acquired { byte: buf[0] })),
                    Ok(_) => {
                        tracing::info!("ERIC: early EOF");
                ...
                loop {
                    tracing::info!("ERIC: polling for events");
                    fd.revents = 0;
...
pub(crate) fn spawn_helper(
    ...
    let thread = Builder::new().spawn(move || {
        tracing::info!("ERIC: jobserver helper thread started");
        state2.for_each_request(|helper| loop {
            match client.inner.acquire_allow_interrupts() {
                Ok(Some(data)) => {
                    tracing::info!("ERIC: helper got token");
                ...
                Err(e) => {
                    tracing::info!("ERIC: helper got error");
                    break f(Err(e));
                }
                Ok(None) if helper.producer_done() => {
                    tracing::info!("ERIC: producer done, exiting");
                    break;
                }
                Ok(None) => {
                    tracing::info!("ERIC: got EINTR, try again");
                }
            }
        });
    })?;
